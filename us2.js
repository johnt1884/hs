...
function createQuoteElement(quotedMessageObject, boardForLink) {
    const quotedElement = document.createElement('div');
    quotedElement.style.borderLeft = '2px solid #ccc';
    quotedElement.style.paddingLeft = '10px';
    quotedElement.style.marginLeft = '10px';
    quotedElement.style.fontStyle = 'italic';
    quotedElement.textContent = `>>${quotedMessageObject.id}\n${quotedMessageObject.text}`;
...
function appendTextOrQuoteSegment(textElement, segment, quoteRegex, currentDepth, MAX_QUOTE_DEPTH, messagesByThreadId, uniqueImageViewerHashes, boardForLink, mediaLoadPromises, parentMessageId) {
    ...
    if (quoteMatch && segment.startsWith(quoteMatch[0])) { // Process as quote only if segment starts with it
        ...
        if (quotedMessageObject) {
            const quotedElement = createQuoteElement(quotedMessageObject, quotedMessageObject.board || boardForLink);
            if (quotedElement) {
...
function _populateAttachmentDivWithMedia(
    ...
    async function createMessageElementDOM(message, mediaLoadPromises, uniqueImageViewerHashes, boardForLink, isTopLevelMessage, currentDepth, threadColor, parentMessageId = null) {
        ...
        const allThemeSettings = await GM.getValue(THEME_SETTINGS_KEY, {});
        let depthKeyPart;
    ...
    function toggleViewer(options = { renderMessages: true }) {
        if (!otkViewer) {
...
async function saveThemeSetting(key, value, requiresRerender = false) {
    if (requiresRerender) {
        pendingThemeChanges[key] = value;
        showApplyDiscardButtons();
    } else {
        let settings = await GM.getValue(THEME_SETTINGS_KEY, {});
        if (value === null || value === undefined) {
            delete settings[key];
        } else {
            settings[key] = value;
        }
        await GM.setValue(THEME_SETTINGS_KEY, settings);
...
function setupOptionsWindow() {
    ...
    setAsMainThemeButton.style.cssText = "width: 100%; padding: 4px 8px; font-size: 11px; height: 25px; box-sizing: border-box; grid-column: 2 / 4; margin-top: 5px;";
    setAsMainThemeButton.addEventListener('click', async () => {
        const themeName = "Main";
        const currentSettings = await GM.getValue(THEME_SETTINGS_KEY, {});
        await GM.setValue(themeName, currentSettings);
    ...
    customThemeActionsWrapper.appendChild(setAsMainThemeButton);
    ...
    saveThemeButton.addEventListener('click', async () => {
        ...
        const currentSettings = await GM.getValue(THEME_SETTINGS_KEY, {});
        let allCustomThemes = await GM.getValue(CUSTOM_THEMES_KEY, {});
        allCustomThemes[themeName] = currentSettings;
        await GM.setValue(CUSTOM_THEMES_KEY, JSON.stringify(allCustomThemes));

        alert(`Theme "${themeName}" saved!`);
        populateCustomThemesDropdown();
    });

    async function populateCustomThemesDropdown() {
        ...
        const customThemes = await GM.getValue(CUSTOM_THEMES_KEY, []);
    ...
    customThemesDropdown.addEventListener('change', async () => {
        ...
        if (selectedValue === "__REVERT__") {
            if (prePreviewSettings) {
                ...
                await GM.setValue(THEME_SETTINGS_KEY, prePreviewSettings);
                await applyThemeSettings();
                ...
                const activeSettings = await GM.getValue(THEME_SETTINGS_KEY, {});
                await GM.setValue(THEME_SETTINGS_KEY, activeSettings); // Re-affirm
                await applyThemeSettings();
                currentlyPreviewingThemeName = null;
            }
            ...
            if (themeToPreview) {
                if (currentlyPreviewingThemeName === null) { // Only store pre-preview if not already previewing
                    ...
                    prePreviewSettings = await GM.getValue(THEME_SETTINGS_KEY, {});
                ...
                await GM.setValue(THEME_SETTINGS_KEY, themeToPreview.settings); // Temporarily set for applyThemeSettings
                await applyThemeSettings();
    ...
    loadThemeButton.addEventListener('click', async () => {
        ...
        await resetAllThemeSettingsToDefault(false); // Pass false to prevent confirmation prompt

        // 2. Set the new theme's settings into the active settings key.
        await GM.setValue(THEME_SETTINGS_KEY, themeToLoad.settings);
    ...
    deleteThemeButton.addEventListener('click', async () => {
        ...
        let customThemes = await GM.getValue(CUSTOM_THEMES_KEY, []);
        ...
        await GM.setValue(CUSTOM_THEMES_KEY, updatedThemes);
        await populateCustomThemesDropdown(); // Refresh the dropdown
    ...
    async function resetAllThemeSettingsToDefault(promptUser = true) {
        ...
        await GM.setValue(THEME_SETTINGS_KEY, {});
    ...
    closeButton.addEventListener('click', async () => {
        ...
        if (prePreviewSettings) {
            ...
            await GM.setValue(THEME_SETTINGS_KEY, prePreviewSettings);
            await applyThemeSettings(); // Apply the restored settings
...
async function main() {
    ...
    await setupOptionsWindow(); // Call to create the options window shell and event listeners
    const mainTheme = await GM.getValue('Main');
    if (mainTheme) {
        await GM.setValue(THEME_SETTINGS_KEY, mainTheme);
    }
    await applyThemeSettings(); // Apply any saved theme settings
    ...
    try {
        await initDB();
            ...
            consoleLog("OTK Thread Tracker script initialized and running.");
             if (!mainTheme) {
                // The options window is now open by default, so no action is needed here.
            }
